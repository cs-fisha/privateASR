...
DEFAULT_CORRECTOR_PROMPT = (
    ...
    "请理解原意后在不偏离原意太多和不增加太多篇幅的前提下，整体对文本进行润色，"
    "确保语义通顺、逻辑清晰、表达自然。"
)
...
async def _correct(text: str, enabled: bool = True) -> tuple[str, str]:
    ...
    if not CORRECTOR_URL or not CORRECTOR_MODEL:
        return text, "unconfigured"
    headers = {"content-type": "application/json"}
    if CORRECTOR_KEY:
        headers["authorization"] = f"Bearer {CORRECTOR_KEY}"
    body = {"model": CORRECTOR_MODEL, "temperature": CORRECTOR_TEMPERATURE,
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=CORRECTOR_TIMEOUT,
            ...
            response = await client.post(f"{CORRECTOR_URL}/chat/completions", json=body, headers=headers)
            response.raise_for_status()
            corrected = response.json()["choices"][0]["message"]["content"].strip()
            return (corrected or text), "applied"
    except Exception as exc:
...
def health() -> dict[str, Any]:
    return {"ok": True, "ready": _engine is not None, "engine": _engine_name,
            ...
            "corrector_configured": bool(CORRECTOR_URL and CORRECTOR_MODEL)}
