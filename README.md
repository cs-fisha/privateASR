...
录音、二次润色和撤销快捷键均可在设置窗口修改并立即生效；留空、格式错误或彼此重复时不会保存。默认分别为 `Ctrl+Alt+Space`、`Ctrl+Alt+P` 和 `Ctrl+Alt+Backspace`。撤销快捷键会向当前获得焦点的程序发送一次 `Ctrl+Z`，因此应在刚完成语音输入且焦点仍在原输入框时使用。

关闭设置窗口只会把客户端收入系统托盘，托盘菜单可以重新打开窗口、切换录音或彻底退出。“启动后直接进入托盘”可在设置中启用。录音开始、录音结束、识别结果、错误与撤销状态会显示在屏幕右下角，并自动消失。
